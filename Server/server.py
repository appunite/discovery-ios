...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
  ...
  def on_message(self, msg):
    ...
    if header["type"] == "absence":
      print "Recived `absence` message: %s" % (body_id)
      
      if not body_id in metadatas:
        return

      for client in clients:
        if client.connection == self:
          client.relations.remove(body_id)

    # handle `presence`
    if header["type"] == "presence":
      print "Recived `presence` message: %s" % (body_id)
      if not body_id in metadatas:
        return

      payload = json.dumps({"header": {"type": "metadata"}, "body": metadatas[body_id]})
      for client in clients:
        if client.connection == self:
          client.relations.add(body_id)
